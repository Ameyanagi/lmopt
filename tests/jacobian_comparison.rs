    ...
    for (i, (x, y)) in test_points.iter().enumerate() {
        ...
        for row in 0..4 {
            for col in 0..2 {
                ...
                println!(
                    "Test point {}, element ({},{}): analytical={:.8}, forward={:.8} (err={:.8}), central={:.8} (err={:.8}), backward={:.8} (err={:.8})",
                    i,
                    row,
                    col,
                    analytical[(row, col)],
                    forward_jacobian[(row, col)],
                    forward_error,
                    central_jacobian[(row, col)],
                    central_error,
                    backward_jacobian[(row, col)],
                    backward_error
                );

                // Define tolerances
                let strict_tolerance = 1e-7;
                let loose_tolerance = 1e-5;

                // Central difference should be most accurate
                assert_relative_eq!(central_jacobian[(row, col)], analytical[(row, col)], max_relative = strict_tolerance, epsilon = strict_tolerance);

                // Forward and backward differences should be reasonably accurate
                assert_relative_eq!(forward_jacobian[(row, col)], analytical[(row, col)], max_relative = loose_tolerance, epsilon = loose_tolerance);

                assert_relative_eq!(backward_jacobian[(row, col)], analytical[(row, col)], max_relative = loose_tolerance, epsilon = loose_tolerance);
            }
        }
    }
}
